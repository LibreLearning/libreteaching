...
\alert{class} (mandatory): book, article, report, letter, slides\dots \\
\alert{package} (optional): To allow using external macros. 
...
\tiny
(Related to font size in \texttt{\textbackslash documentclass})
...
\item Text aligned
\item $n > 1$ blank lines and empty spaces: \alert{one} line or \alert{one} space
...
\begin{frame} 
\frametitle{Example final output}

\begin{block}{Example final output}
